// this module is from tigerbeetle's abstraction over all IO operations. At the time
// of writing this, the Tigerbeetle source code was licensed under the Apache 2.0 license. Here is permalink to license https://github.com/tigerbeetle/tigerbeetle/blob/52723c7e537383e66c9e64a17f22a25f6eadb171/LICENSE
// for more information see https://github.com/tigerbeetle/tigerbeetle/blob/main/src/io/linux.zig
...
const DoublyLinkedListType = @import("../data_structures/list.zig").DoublyLinkedListType;
...
pub const IO = struct {
    ...
    pub fn init(entries: u16, flags: u32) !IO {
        ...
        const version = try utils.parse_dirty_semver(&uts.release);
            ...
            switch (completion.operation) {
                    ...
                    const result: CancelError!void = result: {
                        if (completion.result < 0) {
                            break :result switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                // SQE is invalid.
                                .INVAL => unreachable,
                                else => |errno| utils.unexpected_errno("cancel", errno),
                            };
                        }
                    };
                    ...
                    const result: AcceptError!posix.socket_t = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                else => |errno| utils.unexpected_errno("accept", errno),
                            };
                            break :blk err;
                        } else {
                    ...
                    const result: CloseError!void = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                else => |errno| utils.unexpected_errno("close", errno),
                            };
                            break :blk err;
                        } else {
                    ...
                    const result: ConnectError!void = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                else => |errno| utils.unexpected_errno("connect", errno),
                            };
                            break :blk err;
                        } else {
                            assert(completion.result == 0);
                    ...
                    const result: OpenatError!fd_t = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                else => |errno| utils.unexpected_errno("openat", errno),
                            };
                            break :blk err;
                        } else {
                            break :blk @intCast(completion.result);
                    ...
                    const result: ReadError!usize = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .TIMEDOUT => error.ConnectionTimedOut,
                                else => |errno| utils.unexpected_errno("read", errno),
                            };
                            break :blk err;
                        } else {
                            break :blk @intCast(completion.result);
                    ...
                    const result: RecvError!usize = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .TIMEDOUT => error.ConnectionTimedOut,
                                .OPNOTSUPP => error.OperationNotSupported,
                                else => |errno| utils.unexpected_errno("recv", errno),
                            };
                            break :blk err;
                        } else {
                            break :blk @intCast(completion.result);
                    ...
                    const result: SendError!usize = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .TIMEDOUT => error.ConnectionTimedOut,
                                else => |errno| utils.unexpected_errno("send", errno),
                            };
                            break :blk err;
                        } else {
                            break :blk @intCast(completion.result);
                    ...
                    const result: StatxError!void = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .NOMEM => error.SystemResources,
                                .NOTDIR => error.NotDir,
                                else => |errno| utils.unexpected_errno("statx", errno),
                            };
                            break :blk err;
                        } else {
                            assert(completion.result == 0);
                ...
                .timeout => {
                    ...
                    const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                        ...
                        .TIME => {}, // A success.
                        else => |errno| utils.unexpected_errno("timeout", errno),
                    };
                    ...
                    const result: WriteError!usize = blk: {
                        if (completion.result < 0) {
                            const err = switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                .PIPE => error.BrokenPipe,
                                .SPIPE => error.Unseekable,
                                else => |errno| utils.unexpected_errno("write", errno),
                            };
                            break :blk err;
                        } else {
                            break :blk @intCast(completion.result);
        ...
        switch (kind) {
            ...
            .block_device => {
                ...
                switch (os.linux.E.init(os.linux.ioctl(
                    ...
                    else => |err| return utils.unexpected_errno("open_file:ioctl", err),
                }
                ...
                // NOTE: in order to use this method, we should implement this ourselves. As of today, I don't
    ...
    fn fs_is_tmpfs(dir_fd: fd_t) !bool {
        var statfs: utils.StatFs = undefined;

        while (true) {
            const res = utils.fstatfs(dir_fd, &statfs);
            switch (os.linux.E.init(res)) {
                .SUCCESS => {
                    return statfs.f_type == utils.TmpfsMagic;
                },
                .INTR => continue,
                else => |err| return utils.unexpected_errno("fs_is_tmpfs", err),
            }
        }
    }
    ...
    fn fs_supports_direct_io(dir_fd: fd_t) !bool {
        ...
        _ = utils.array_print(16, &cookie, "{0x}", .{std.crypto.random.int(u64)});
        ...
        while (true) {
            ...
            switch (os.linux.E.init(res)) {
                ...
                .INTR => continue,
                .INVAL => return false,
                else => |err| return utils.unexpected_errno("fs_supports_direct_io", err),
            }
        }
    }
    ...
    fn fs_allocate(fd: fd_t, size: u64) !void {
        ...
        while (true) {
            ...
            switch (os.linux.E.init(rc)) {
                ...
                else => |errno| return utils.unexpected_errno("fs_allocate", errno),
            }
        }
    }
};
