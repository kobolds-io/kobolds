...
const SocketOpts = struct {
    no_delay: i32 = 1,

    pub fn validate(self: SocketOpts) ?[]const u8 {
        if (self.no_delay > 1) return "invalid no_delay";
...
pub const Dialer = struct {
    ...
    const Config = struct {
        ...
        pub fn validate(self: Config) ?[]const u8 {
            if (self.host.len == 0) return "invalid host";
            if (self.port == 0) return "invalid port";
            if (self.port > std.math.maxInt(u16)) return "invalid port";

            if (self.socket_opts.validate()) |reason| return reason;
    ...
    pub fn init(config: Config) !Self {
        if (config.validate()) |reason| {
            log.err("invalid config: {s}", .{reason});
            return error.InvalidDialerConfig;
        }
...
pub const Listener = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, id: usize, config: Config) !Self {
    ...
    pub fn tick(self: *Self, listener_socket: posix.socket_t) !void {
        self.handleAccept(listener_socket);
    }

    pub fn listen(self: *Self, ready_channel: *UnbufferedChannel(bool)) void {
    ...
    fn handleAccept(self: *Self, listener_socket: posix.socket_t) void {
        if (self.accept_submitted) return;
    ...
    pub fn onAccept(self: *Self, comp: *IO.Completion, res: IO.AcceptError!posix.socket_t) void {
        ...
        const unspecified_address = net.Address.parseIp("0.0.0.0", 0) catch @panic("failed to parse zero'd IP");
        ...
        if (self.config.socket_options.no_delay > 0) {
            posix.setsockopt(socket, posix.IPPROTO.TCP, posix.TCP.NODELAY, &std.mem.toBytes(@as(c_int, self.config.socket_options.no_delay))) catch |err| {
                log.err("failed to set posix.setsockopt: {any}", .{err});
...
test "tcp listener usage" {
    const allocator = testing.allocator;

    var listener = try Listener.init(allocator, 0, .{
        .port = 9898,
    });
    defer listener.deinit();

    // ready channel to be notified when the listener thread is up and running
    var ready_channel = UnbufferedChannel(bool).new();

    // spawn a thread to run the listener
    const thread = try std.Thread.spawn(.{}, Listener.listen, .{ &listener, &ready_channel });
    errdefer listener.close();

    _ = try ready_channel.tryReceive(500 * std.time.ns_per_ms);
